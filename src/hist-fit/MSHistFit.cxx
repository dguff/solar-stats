...
namespace mst
{
                          ...
                          const int nPts1, const int nPts2)
   {
      ...
      if (!poi1 || !poi2)
      {
         std::cerr << "Profile >> error: one or both parameters not found\n";
         return nullptr;
      }
      ...
      for (auto it : *fitter->GetParameterMap())
      {
         fitBestValue[parIndex] = it.second->GetFitBestValue();
         fitBestValueErr[parIndex] = it.second->GetFitBestValueErr();
         parIndex++;
      }
      ...
      auto Scan = [&](double t1Val, double t2Val)
      {
         ...
         if (tmpMinNLL < absMinNLL)
            absMinNLL = tmpMinNLL;
         return tmpMinNLL - absMinNLL <= NLL;
      };
      ...
      for (auto it : *fitter->GetParameterMap())
      {
         it.second->SetFitBestValue(fitBestValue[parIndex]);
         it.second->SetFitBestValueErr(fitBestValueErr[parIndex]);
         parIndex++;
      }
      poi1->Release();
      poi2->Release();
