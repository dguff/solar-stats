...
namespace mst
{
  ...
  inline TH1* AdjustHist(const TH1* target, const MSTHnHandler::axis& handler_axis) {
    ...
    if (dynamic_cast<const TH1D*>(target) == nullptr) {
      fprintf(stderr, "AdjustHist error: target hist is not of type TH1D\n");
      exit(EXIT_FAILURE);
    }

    const int nbins = handler_axis.fNbins;
    const double min = handler_axis.fMin; 
    const double max = handler_axis.fMax;

    TH1D* h = new TH1D(Form("%s_adjusted", target->GetName()), target->GetTitle(), nbins, min, max);

    TF1 fun("fun", [&](double* x, double* p) {return target->Interpolate(x[0]);}, min, max, 0);
    fun.SetNpx(nbins*10);

    for (int i = 1; i <= nbins; i++) {
      double x0 = h->GetBinLowEdge(i);
      double x1 = h->GetBinLowEdge(i+1);
      double bc = fun.Integral(x0, x1);
      h->SetBinContent(i, bc);
    }
  ...
  inline rapidjson::Document LoadConfig(const string &configFileName, bool verbose = false)
  {
                                      ...
                                      const int memberSize = 0)
    {
      // verbose dump
      if (verbose)
        cout << "info: checking json config file: member " << memberName << "... ";
      ...
      {
        bool c = false;
        if (!strcmp(memberType.c_str(), "Bool"))
          c = member.IsBool();
        else if (!strcmp(memberType.c_str(), "Number"))
          c = member.IsNumber();
        else if (!strcmp(memberType.c_str(), "Int"))
          c = member.IsInt();
        else if (!strcmp(memberType.c_str(), "Double"))
          c = member.IsDouble();
        else if (!strcmp(memberType.c_str(), "String"))
          c = member.IsString();
        else if (!strcmp(memberType.c_str(), "Array"))
          c = member.IsArray();
        else if (!strcmp(memberType.c_str(), "Object"))
          c = member.IsObject();
        return c;
      };
    ...
    { // json/fittingModel/dataSets/*
      if (verbose)
        cout << "info: checking dataSet "                     //
      ...
      {                                                               // optional block:
        ...
        { // json/fittingModel/dataSets/*/detectorResponse/*
          if (verbose)
            cout << "info: checking detectorResponse "                       //
      ...
      { // json/fittingModel/dataSets/*/components/*
        if (verbose)
          cout << "info: checking component "                            //
        ...
        if (std::strcmp(type.c_str(), "neutrino") == 0)
        {
          ...
          for (const auto &channel : component.value["channels"].GetObject())
          {                                                             // json/fittingModel/dataSets/*/components/*/channels/*
      ...
      { // json/fittingModel/dataSets/*/axis/*
        if (verbose)
          cout << "info: checking axis "                            //
      ...
      if (dataSet.value.HasMember("oscillation_handler")) {
        isMemberCorrect(dataSet.value, "oscillation_handler", "Object");
        const auto& oscillationHandler = dataSet.value["oscillation_handler"];
        isMemberCorrect(oscillationHandler, "axis", "Object");
        for (const auto& axis : oscillationHandler["axis"].GetObject()) {
          if (verbose)
            cout << "info: checking axis "                            //
              << axis.name.GetString() << endl;                    //
                                         ...
                                         const std::string &datafileName)
  {
    ...
    for (const auto &dataSet : jfittingModel["dataSets"].GetObject())
    {
      ...
      // Set the oscillation handler if needed
      if (dataSet.value.HasMember("oscillation_handler")) {
      ...
      if (json.HasMember("MC"))
        pdfBuilder->SetSeed(json["MC"]["seed"].GetInt());
      ...
      for (const auto &component : dataSet.value["components"].GetObject())
      {
        ...
        // otherwise use the range to guess to the the fit starting step
          ...
          par->SetFitStartStep(((component.value["range"].GetArray())[1].GetDouble() - (component.value["range"].GetArray())[0].GetDouble()) / 100.);
  ...
  inline bool SetDataSetFromMC(const rapidjson::Document &json, MSMinimizer *fitter)
  {
    ...
    for (const auto &model : *fitter->GetModels())
    {
      ...
      mod->SetDataSet(pdfBuilder->GetMCRealizaton(
          ...
          sampling_procedure));
      // std::cout << "info: data set for model " << mod->GetName() << " created\n";
    }
    return true;
  }
          ...
          if (pdfBuilder->IsNeutrino(parName))
          {
            ...
            for (const auto &channel : channels)
            {
              ...
              pdfBuilder->AddHistToPDF(parName.c_str(),
                                       channel.fName.data(), par->GetFitBestValue(), fitter->GetNeutrinoPropagator());
              auto pdf_tmp = pdfBuilder->GetPDF("");
              pdf_tmp->Scale( mod->GetExposure() ); 
              ...
              int color = json["fittingModel"]["dataSets"][mod->GetName().c_str()]
                              ["components"][parName.c_str()]["color"]
                                  .GetInt();
                         ...
                         const string &parName, const double NLL, const int nPts)
  {
    ...
    {
      if (tVal < poi->GetRangeMin() || tVal > poi->GetRangeMax())
        return false;
      poi->FixTo(tVal);
