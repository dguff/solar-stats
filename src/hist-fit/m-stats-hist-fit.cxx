...
int main(int argc, char **argv)
{
   ...
   if (gOperationMode == EOperationMode::kInteractiveFit)
   {
      ...
      if (gInputFileName.empty())
         mst::SetDataSetFromMC(json, fitter);
   ...
   else if (gOperationMode == EOperationMode::kBatchFit)
   {
      ...
      if (gAppendOnFile == false || otree == nullptr)
      {
         ...
         if (gStoreMFCanvasMLF)
            otree->Branch("cMLF", &cMLF);
         if (gBuildProfiles)
            otree->Branch("cPLL", &cPLL);
      }
      else
      {
         ...
         if (gStoreMFCanvasMLF)
            otree->SetBranchAddress("cMLF", &cMLF);
         if (gBuildProfiles)
            otree->SetBranchAddress("cPLL", &cPLL);
      }
      ...
      for (int i = 0; i < iMax; i++)
      {
         ...
         if (gStoreMFCanvasMLF)
            cMLF = mst::GetCanvasFit(json, fitter);
         if (gBuildProfiles)
            cPLL = mst::GetCanvasProfiles(json, fitter,
                                          TMath::ChisquareQuantile(gProfilesCL, 1),
                                          gProfilePts);
